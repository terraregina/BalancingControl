...
def create_trials_planning(data, habit_seq = 3, contingency_degradation = True,\
    ...
    fname = 'new_planning_config_'+'degradation_'+ str(int(contingency_degradation))+ '_switch_' + str(int(switch_cues))\
...
# fname = 'config/shuffled_and_blocked/new_planning_config_degradation_1_switch_0_train5_degr0_n42_nr_3.json'
fname = 'config/shuffled_and_blocked/new_planning_config_degradation_1_switch_0_train2_degr2_n42_nr_3.json'
...
exp_rewards = df.groupby(by=['block','context']).mean('exp_reward')['exp_reward']
# exp_rewards
total_reward = df.groupby(by=['trial_type'])['exp_reward'].sum()
total_reward

#%% Minimum points border
...
trials = np.array(df['planets'].to_list())
trials = rewards[trials]
starts = df['starts'].to_numpy()
stm = np.array(df.loc[0]['state_transition_matrix'])
...
# for t in range(5):
    ...
    start = starts[t]
    for a in range(3):
        # print('\nt, a, start, trial',t,a,start, trial)
        move = (stm[start, :,0]*trial).sum()
        jump = (stm[start, :,1]*trial).sum()
        reward = [move,jump]
        # print('move and jump points:', move, jump)
        ...
        # print('was at', start)
        start = np.argmax(stm[start, :,i])
        # print('now at', start)

print('During Day 1 if you only go for the immediate best planet you would gather: ', points)
#%% OLD FUNCTIONS
