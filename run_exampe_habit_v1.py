...
trial_type = np.asarray(data['trial_type'])
blocks = np.asarray(data['block'])
...
def run_agent(par_list, trials, T, ns=6, na=2, nr=3, nc=2, npl=2, deval=False):
    ...
    learn_pol, context_trans_prob, avg, Rho, utility, B, planets, starts, colors, rc = par_list
    ...
    environment.true_optimal = sequence
    environment.trial_type = trial_type
    """ 
