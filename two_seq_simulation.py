...
#     colors = ar.asarray(task_params['context'])          # 0/1 as indicator of color
#     sequence = ar.asarray(task_params['sequence'])       # what is the optimal sequence
#     starts = ar.asarray(task_params['starts'])           # starting position of agent
#     planets = ar.asarray(task_params['planets'])         # planet positions 
#     trial_type = ar.asarray(task_params['trial_type'])
#     blocks = ar.asarray(task_params['block'])
...
#     all_optimal_seqs = ar.unique(sequence)                                                                            
...
#     u = 0.99
#     utility = ar.array([(1-u)/2,(1-u)/2,u])

#     if reward_naive==True:
#         reward_counts = ar.ones([nr, npl, nc])
#     else:
#         reward_counts = ar.tile(planet_reward_probs.T[:,:,ar.newaxis]*20,(1,1,nc))+1
...
#                                         [0.05, 0.95 , 0.05],
#                                         [0.95, 0.05 , 0.0]]).T 
# state_transition_matrix = ar.zeros([ns,ns,na])
# m = [1,2,3,4,5,0]
# for r, row in enumerate(state_transition_matrix[:,:,0]):
#     row[m[r]] = 1
...
# state_transition_matrix = transpose(state_transition_matrix, (1,0,2))
# # state_transition_matrix = ar.repeat(state_transition_matrix[:,:,:, None], repeats=nc, axis=3)
# state_transition_matrix = transpose(ar.stack([state_transition_matrix for i in range(nc)]), (1,2,3,0))
